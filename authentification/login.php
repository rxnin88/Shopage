...
<body>
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $r = "select * from users where email ='" . $email . "' and password = '" . $hashedPass . "'";
    require("../connexion.php");
    $res = mysqli_query($con, $r);
    $data = mysqli_fetch_assoc($res);
    $nbrl = mysqli_num_rows($res);
    mysqli_close($con);
    if ($nbrl == 0)
      echo '
      <div class="border-b border-1 border-red-500 top-4 px-4 py-3 rounded" role="alert">
        <strong class="font-bold">Login failed!</strong>
        <span class="block sm:inline">Please check your email and password and try again.</span>
      </div>
    ';
    if ($nbrl == 1) {
      $_SESSION['login'] = $email;
      $_SESSION['password'] = sha1($password);
      $_SESSION['cle'] = $data['id'];
    ...
    <form action="login.php" method="POST" class="mx-auto mb-0 mt-8 max-w-md space-y-4">
      ...
      <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500">
          No account?
          <a class="underline" href="/authentification/register.php">Sign up</a>
        </p>

        <button type="submit" class="inline-bloc w-full px-5 py-2 mb-2 text-lg text-white bg-black rounded-lg sm:w-auto sm:mb-0 hover:bg-green-500 transition duration-300 ease-in-out">
          Sign in
        </button>
      </div>
    </form>
